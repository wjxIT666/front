...
import { isEmpty, lengthRule, isCountEmpty } from '../../js/rules.js';
import { AccessId, Channel, Version, API } from '../../request/config.js';
import { getSign } from '../../request/sign.js';
import { addGame } from '../../request/api.js';
import axios from 'axios';
const { Item } = Form;
const { Option } = Select;

export default class X86_Modal extends Component {
    ...
    modalOk = async () => {
        let { applyNameVal, applyPath, applyCountVal, resourceCode } = this.state;
        ...
        let isCountEmptyError = isCountEmpty(applyCountVal, '并发数不能为空');
        if (!isEmptyError || !lengthRuleError || !isEmptyapplyPath || !isCountEmptyError) return false;
        let data = {name: applyNameVal, packageName: applyPath, concurrency: applyCountVal, appType: 1, resourceCode: resourceCode };
        //调用新增游戏接口
        let addGameRes = await addGame(data);
        console.log(addGameRes)
        if (addGameRes.result === true) {
        ...
        } else if (addGameRes.result === false) {
            message.error('创建失败');
        };
    };
    applyNameVal = (e) => {
    ...
    applyCountVal = (e) => {
        this.setState({
            applyCountVal: parseInt(e.target.value)
        });
    };
    componentDidMount () {
    ...
    render () {
        ...
        let { modalText, modalVisible, modalOkText, modalCancelText } = this.props;
        const UploadProps = {
            name: 'app',
            maxCount: 1,
            multiple: false,
        ...
        return (
            <div>
                <Modal title={modalText} visible={modalVisible} cancelText={modalCancelText} okText={modalOkText} onCancel={this.modalCancel} onOk={this.modalOk} destroyOnClose={true}>
                    <Form autoComplete="off" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }} preserve={false}>
                        ...
                        <div>
                            ...
                            <Item
                                ...
                                name="applyPath"
                            >
                                <Select placeholder={this.searchPlaceholder} onChange={this.applyPathChange}>
                                    {
                            ...
                            {/* 并发数 */}
                            <Item
                                label={<span><span style={{color: 'red'}}>*</span> 并发数</span>}
                                name="applyCount"
                            >
                                <Input onChange={this.applyCountVal}/>
                            </Item>
                        </div>
                    </Form>
                </Modal>
