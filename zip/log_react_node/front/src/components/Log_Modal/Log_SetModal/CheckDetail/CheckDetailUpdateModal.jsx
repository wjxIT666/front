...
import { checkDetailEdit } from '../../../../request/api.js';
import { isEmpty } from '../../../../js/util.js';
const { Item } = Form;

export default class CheckDetailUpdateModal extends Component {
    state = {
        name: '',
        address: '',
        url: '',
    ...
    handleOk = async () => {
        ...
        let data = { name, address, url, port, path, id, key };
        let checkDetailEditRes = await checkDetailEdit(data);
        let { code, msg } = checkDetailEditRes;
        if (code === 0) {
            message.success(msg);
            this.setState({
                name: '',
            ...
            this.props.handleUpdateOk(false);
        };
    };
    //取消
    handleCancel = () => {
