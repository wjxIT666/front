<template>
    <div class="plan_add">
        <!-- 游戏新增弹窗 -->
        <el-dialog title="计划创建" custom-class="change_dialog" :visible.sync="centerDialogVisible" width="60%" center :hide-required-asterisk="true" :close-on-click-modal="false" :close-on-press-escape="false" :append-to-body="true" @close="closePlan" :lock-scroll="false">
            <!-- 表单 -->
...
export default {
        ...
        commit (formRule, form) {
            ...
            this.$refs[formRule].validate( async (valid) => {
                if (valid) {
                    ...
                    //关闭弹窗
                    this.$emit('down-plan', this.dataItem, this._props.formItem.id);
                } else {
                    //验证未通过
                    return false;
                };
    ...
    mounted () {
        this.gameId = this._props.formItem.gameId;
    }
}
</script>
